...
import { FeatureProductVisual } from './FeatureProductVisual';
...
export type ServiceCardContentProps = {
  ...
  stackPeek?: boolean;
  /** Shown only on the active card, centered between headline and footer rail. */
...
export function ServiceCardContent({
  ...
  const showProduct = Boolean(isActive && productImageSrc);
    ...
    <div className="relative z-10 flex h-full min-h-0 flex-col">
      ...
      <div
        className={cn(
          ...
          showProduct && 'flex min-h-[128px] flex-col justify-center py-2 sm:py-3 lg:min-h-0 lg:py-2',
        )}
      >
        {showProduct && productImageSrc ? (
          <FeatureProductVisual src={productImageSrc} title={title} compact={compact} />
        ) : null}
      </div>

      <div className={`mt-auto shrink-0 ${compact ? 'pt-5' : 'pt-6'}`}>
        <motion.div
